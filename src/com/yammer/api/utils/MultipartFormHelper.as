package com.yammer.api.utils
{
	...
	public class MultipartFormHelper 
	{
		 ...
		 */
		public static function getPostData(files:Array, parameters:Object = null):ByteArray 
		{
			var i: int;
			var bytes:String;
			...
			while (++j < files.length) 
			{
				...
				byteArray = files[j].bytes;	
				
				//add Filedata to postData
				postData = BOUNDARY(postData);
				postData = LINEBREAK(postData);
				bytes = 'Content-Disposition: form-data; name="attachment' + (j + 1) + '"; filename="';
				...
				bytes = ( files[j].image ) ? 'Content-Type: image/jpeg' : 'Content-Type: application/octet-stream';
		...
		{
			var l:int = MultipartFormHelper.getBoundary().length;
